...
When the user wants to explore alternative interfaces for a chosen deepening candidate, use this independent-alternatives process. Based on "Design It Twice" (Ousterhout) — your first idea is unlikely to be the best.
...
Show this to the user, then immediately proceed to Step 2. The user reads and thinks while you produce alternatives independently from the original brief.

### 2. Produce independent alternatives

Produce 3+ alternatives. Each must be a **radically different** interface for the deepened module. Work from the original technical brief each time; do not let one alternative anchor the next.
