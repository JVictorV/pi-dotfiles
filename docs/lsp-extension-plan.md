...
- **Spawn permission**: Confirm before starting an LSP server for a repository, then store the user's `allow` or `deny` preference globally per repository absolute path so future sessions do not repeat the prompt. The repository path is the canonical Git root when available, otherwise the canonical session `cwd`. Provide commands to inspect, allow, deny, and reset preferences.
